...
import type { SecurityStatus as SystemStatusType } from "@/types/dashboard"
import Image from "next/image"
import { cva, type VariantProps } from "class-variance-authority"
import { cn } from "@/lib/utils"
import { Bullet } from "@/components/ui/bullet"

const systemStatusItemVariants = cva("border rounded-md ring-4", {
  variants: {
    variant: {
...
interface SystemStatusItemProps extends VariantProps<typeof systemStatusItemVariants> {
  title: string
  value: string
  status: string
  className?: string
...
function SystemStatusItem({ title, value, status, variant, className }: SystemStatusItemProps) {
  return (
    <div className={cn(systemStatusItemVariants({ variant }), className)}>
...
export default function SystemStatus({ statuses }: SystemStatusProps) {
  return (
    <DashboardCard title="SYSTEM STATUS" intent="success" addon={<Badge variant="outline-success">ONLINE</Badge>}>
